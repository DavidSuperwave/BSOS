...
export async function POST(
  ...
  try {
    // Official endpoint in current docs.
    let res = await fetch(`${PLUSVIBE_BASE}/campaign/launch`, requestInit);

    // Backward compatibility for older PlusVibe deployments.
    if (!res.ok && res.status === 404) {
      res = await fetch(`${PLUSVIBE_BASE}/campaign/activate`, requestInit);
    }
    ...
    return NextResponse.json(
      {
        error: err?.message || "Failed to activate campaign",
        code: "PLUSVIBE_ERROR",
      },
      { status: 500 }
    );
