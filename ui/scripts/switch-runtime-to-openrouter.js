...
async function updateTarget(target) {
  ...
  updated = setOrInsertEnvLine(updated, "OPENROUTER_API_KEY", OPENROUTER_API_KEY);
  updated = setOrInsertEnvLine(updated, "ANTHROPIC_API_KEY", OPENROUTER_API_KEY);
  updated = setOrInsertEnvLine(
    updated,
    "ANTHROPIC_BASE_URL",
  ...
  const verify = await sshExec(
    `docker exec ${target.container} sh -c "echo ANTHROPIC_BASE_URL=$ANTHROPIC_BASE_URL && if [ -n \\"$OPENROUTER_API_KEY\\" ]; then echo OPENROUTER_API_KEY_SET=yes; else echo OPENROUTER_API_KEY_SET=no; fi"`
  );
  if (verify.code !== 0) {
...
async function main() {
  ...
  console.log(`Switching runtime auth to OpenRouter on ${DROPLET_IP}...`);
